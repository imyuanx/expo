...
> **warning** Static URL rewrites are only supported in server-side rendering (SSR) environments. They are
> not available in static exports or native apps. For client-side navigation, consider using [redirects](#redirect-component)
> instead.
